...
export async function once<TTheme extends ThemeBase>(config: Config<TTheme>) {
  ...
  if (config.emitTo.to === "oneFile") {
    ...
    if (result) {
      await writeFile(
        config.emitTo.path,
        `/* THIS FILE IS AUTOMATICALLY GENERATED BY guc-node. */\n${result}`
      );
    }
  } else {
    const filePathsToProcess = await glob(config.filesToProcessGlobPattern);
...
async function processFile<TTheme extends ThemeBase>(
  filePath: string,
