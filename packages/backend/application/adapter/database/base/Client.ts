...
export type GetResult<Model, Get extends GetInput<Model>> = Get extends {
  ...
  : Get extends { include: infer I extends IncludeInput<Model> }
    ...
    : Scalars<Model>;
type WrapArray<Target, Original> = Original extends unknown[]
  ? Target[]
  : Target;
type WrapNullable<Target, Original> =
  Original extends NonNullable<Original> ? Target : Target | undefined;

export type UniqueWhere<Model> = XOR<
  UniqueWhereInternal<Model, Extract<UniqueIndices<Model>, unknown[]>, []>
>;
...
> = Remaining["length"] extends 0
  ...
  : UniqueWhereInternal<
      ...
      [
        ...
        {
          [K in Extract<
            StringJoin<Extract<Remaining[0], string[]>, "_">,
            string
          >]: {
            [K in Extract<
              Remaining[0],
              string[]
            >[number]]: K extends keyof Scalars<Model>
              ? Scalars<Model>[K]
...
// TODO:
export type ObjectArrayWhere<Model> = XOR<[Model, Model]>;
export type NullableObjectArrayWhere<Model> = XOR<[Model, Model]>;
export type SingleObjectWhere<Model> = XOR<[Model, Model]>;
export type NullableSingleObjectWhere<Model> = XOR<[Model, Model]>;
